...
static void fileDefCB(Widget w, WindowInfo *window, caddr_t callData);
static void editDefCB(Widget w, WindowInfo *window, caddr_t callData);
static void searchDefCB(Widget w, WindowInfo *window, caddr_t callData);
...
static void markUnsetAP(Widget w, XEvent *event, String *args, Cardinal *nArgs);
...
static void moveDocumentDialogAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void moveDocumentRightAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
static void moveDocumentLeftAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs);
...
static void fileMenuAP(Widget w, XEvent *event, String *args, Cardinal *nArgs);
static void editMenuAP(Widget w, XEvent *event, String *args, Cardinal *nArgs);
static void searchMenuAP(Widget w, XEvent *event, String *args, Cardinal *nArgs);
...
static void setWrapTextAP(Widget w, XEvent *event, String *args,
    Cardinal *nArgs);
static void setCommandShellAP(Widget w, XEvent *event, String *args,
    Cardinal *nArgs);
...
static XtActionsRec Actions[] = {
    ...
    {"move_document_right", moveDocumentRightAP},
    {"move_document_left", moveDocumentLeftAP},
    ...
    {"file_menu_command", fileMenuAP},
    {"edit_menu_command", editMenuAP},
    {"search_menu_command", searchMenuAP},
    ...
    {"set_command_shell", setCommandShellAP},
...
Widget CreateMenuBar(Widget parent, WindowInfo *window)
{
    ...
    ** "File" pull down menu.
    */
    menuPane = window->fileMenuPane =
            createMenu(menuBar, "fileMenu", "File", 0, &cascade, FULL);
    btn = createMenuItem(menuPane, "new", "New", 'N', doActionCB, "new", SHORT);
    ...
    XtVaSetValues(window->newOppositeItem, XmNuserData, PERMANENT_MENU_ITEM, NULL);
	...
	XtAddCallback(window->prevOpenMenuItem, XmNcascadingCallback,
        ...
        XtVaSetValues(window->prevOpenMenuItem, XmNuserData, PERMANENT_MENU_ITEM, NULL);
    }

    /* 
    ** "Edit" pull down menu.
    */
    menuPane = window->editMenuPane =
            createMenu(menuBar, "editMenu", "Edit", 0, &cascade, FULL);
    window->undoItem = createMenuItem(menuPane, "undo", "Undo", 'U',
    	    doActionCB, "undo", SHORT);
    XtVaSetValues(window->undoItem, XmNuserData, PERMANENT_MENU_ITEM, NULL);
    XtSetSensitive(window->undoItem, False);
    window->redoItem = createMenuItem(menuPane, "redo", "Redo", 'R',
    	    doActionCB, "redo", SHORT);
    XtVaSetValues(window->redoItem, XmNuserData, PERMANENT_MENU_ITEM, NULL);
    ...
    */
    menuPane = window->searchMenuPane =
            createMenu(menuBar, "searchMenu", "Search", 0, &cascade, FULL);
    btn = createFakeMenuItem(menuPane, "markHidden", markCB, window);
    XtVaSetValues(btn, XmNuserData, PERMANENT_MENU_ITEM, NULL);
    btn = createFakeMenuItem(menuPane, "gotoMarkHidden", gotoMarkCB, window);
    XtVaSetValues(btn, XmNuserData, PERMANENT_MENU_ITEM, NULL);
    btn = createFakeMenuItem(menuPane, "gotoMarkShiftHidden", gotoMarkCB, window);
    XtVaSetValues(btn, XmNuserData, PERMANENT_MENU_ITEM, NULL);
    ...
    */
    ...
    createMenuItem(subSubPane, "fileMenu", "File Menu...", 'F',
    	    fileDefCB, window, FULL);
    createMenuItem(subSubPane, "editMenu", "Edit Menu...", 'E',
    	    editDefCB, window, FULL);
    createMenuItem(subSubPane, "searchMenu", "Search Menu...", 'a',
    	    searchDefCB, window, FULL);
#ifndef VMS
...
static void fileDefCB(Widget w, WindowInfo *window, caddr_t callData)
{
    HidePointerOnKeyedEvent(WidgetToWindow(MENU_WIDGET(w))->lastFocus,
            ((XmAnyCallbackStruct *)callData)->event);
    EditFileMenu(WidgetToWindow(MENU_WIDGET(w)));
}
...
static void editDefCB(Widget w, WindowInfo *window, caddr_t callData)
{
    HidePointerOnKeyedEvent(WidgetToWindow(MENU_WIDGET(w))->lastFocus,
            ((XmAnyCallbackStruct *)callData)->event);
    EditEditMenu(WidgetToWindow(MENU_WIDGET(w)));
}
...
static void searchDefCB(Widget w, WindowInfo *window, caddr_t callData)
{
    HidePointerOnKeyedEvent(WidgetToWindow(MENU_WIDGET(w))->lastFocus,
            ((XmAnyCallbackStruct *)callData)->event);
    EditSearchMenu(WidgetToWindow(MENU_WIDGET(w)));
}
...
static void markUnsetAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    ...
    UnsetMark(WidgetToWindow(w), w, args[0][0]);
}
...
static void moveDocumentRightAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
    MoveDocumentRight(WidgetToWindow(w));    
}
static void moveDocumentLeftAP(Widget w, XEvent *event, String *args,
	Cardinal *nArgs)
{
    MoveDocumentLeft(WidgetToWindow(w));    
}
...
static void fileMenuAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    if (*nArgs == 0) {
    	fprintf(stderr,
    		"nedit: file_menu_command requires item-name argument\n");
    	return;
    }
    ...
    HidePointerOnKeyedEvent(w, event);
    DoNamedFileMenuCmd(WidgetToWindow(w), args[0]);
}
...
static void editMenuAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    if (*nArgs == 0) {
    	fprintf(stderr,
    		"nedit: edit_menu_command requires item-name argument\n");
    	return;
    }
    /* Same remark as for macro menu commands (see above). */
    if (event->xany.send_event != MACRO_EVENT_MARKER) {
    ...
    HidePointerOnKeyedEvent(w, event);
    DoNamedEditMenuCmd(WidgetToWindow(w), args[0]);
}
...
static void searchMenuAP(Widget w, XEvent *event, String *args, Cardinal *nArgs)
{
    if (*nArgs == 0) {
    	fprintf(stderr,
    		"nedit: search_menu_command requires item-name argument\n");
    	return;
    }
    /* Same remark as for macro menu commands (see above). */
    if (event->xany.send_event != MACRO_EVENT_MARKER) {
    ...
    HidePointerOnKeyedEvent(w, event);
    DoNamedSearchMenuCmd(WidgetToWindow(w), args[0]);
}
    ...
    Cardinal *nArgs)
{
    ...
    shellName = args[0];
    if (strlen(shellName) > MAXPATHLEN) {
        return;
    }
    ...
    Cardinal *nArgs)
{
    ...
    if (*nArgs > 0) {
        ...
        if (sscanf(args[0], "%d", &newEmTabDist) == 1 &&
            ...
            fprintf(stderr,
                "nedit: set_em_tab_dist requires integer argument < 1000\n");
        }
    }
    else {
...
Widget createMenuSeparator(Widget parent, char *name, int mode)
{
    Widget button;
...
/*!!! the item was removed    	XtSetSensitive(WindowList->closeItem, FALSE); */
    	return;
    }
...
void AddToPrevOpenMenu(const char *filename)
{
    ...
    ReadNEditDB();
     ...
     /* If the name is to be omitted, return */
     if ((fileMask = GetPrevOpenFilesMask()) && *fileMask)
     {
         compiledRE = CompileRE(fileMask, &compileMsg, REDFLT_STANDARD);
         if (compiledRE == NULL)
         {
             fprintf(stderr, "%s\n", compileMsg);
         } else
         {
             if (ExecRE(compiledRE, NULL, filename, NULL, FALSE, '\0', '\0', NULL, NULL, NULL))
             {
...
void WriteNEditDB(void)
{
    ...
    /* set up removal of any files matching prevOpenFilesMask */
    if ((fileMask = GetPrevOpenFilesMask()) && *fileMask)
    {
          compiledRE=CompileRE(fileMask, &compileMsg, REDFLT_STANDARD);
          if (compiledRE == NULL)
    ...
    for (i = 0; i < NPrevOpen; i++)
    {
        /* omitting any files matching prevOpenFilesMask */
        if (NULL == compiledRE  || 0 == ExecRE(compiledRE, NULL, PrevOpen[i], NULL, FALSE, '\0', '\0', NULL, NULL, NULL))
        {
            size_t lineLen = strlen(PrevOpen[i]);

            /*  omitting any files with invalid filenames  */
