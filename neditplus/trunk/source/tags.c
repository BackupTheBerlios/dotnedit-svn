...
static int loadAllTagsFiles(const char **nameList, int search_type);
...
static int showMatchingCalltip( Widget parent, int i );
...
/* 
 * Successive calls of this function return the names
 * of all loaded tags files in *tfName. 
 * For the first call *callData has to be NULL, the following
 * calls have to re-use the last value supplied by this function.
 * Return value: 1 there are further tags file names to be returned later
 *               0 the last name has been returned. Successive calls will
 *                 restart with the first tags file. 
 */
int GetNextTagsFileName(const char ** tfName, void ** callData) 
{
   tagFile *tagsFilePtr =  (tagFile *) *callData;
  
   if(!tagsFilePtr) 
      tagsFilePtr = TagsFileList;
   else
      tagsFilePtr = tagsFilePtr->next;
   
   *callData = tagsFilePtr;
   
   if(tagsFilePtr) {
      *tfName = tagsFilePtr->filename;
      return 1;
   } else {
      *tfName = "";
      return 0;
...
/* Get all the tags or tips (keys/names) from the tags or tips file tFileName
   that extend stub.  If stub is NULL, get all the keys.  
   Returns a pointer to the \n separated list of the keys found or NULL,
   if nothing was found. 
*/
char *listKeys(int search_type, const char *tFileName, const char *stub) {
    tag *t;
    int nameListLen = 0, nameLen = 0, tFileNameID = 0, allocLen = 2048;
    int stubLen, i;
    char *nameList, *nameCut, *ptr, *fillPtr;
    const char **filestoload;
    tag **table;
    tagFile *FileList, *tf;
        
    /* ensure that the requested tags or tips file(s) is(are) loaded */
    filestoload = (const char **) malloc(2 * sizeof(const char *));
    filestoload[0] = tFileName;
    filestoload[1] = NULL;
    loadAllTagsFiles(filestoload, search_type);
    free(filestoload);
    ...
    for (tf = FileList; tf; tf = tf->next) {
        if (strcmp(tFileName, tf->filename)) 
            continue;
        /* We found the file.  So, we have the unique identifier 
           for the file. 
           tagfile->index is a unique integer > 0 for the tips or tags file
           this identifier is also in every tag, so we know the tags or tips 
           file the tag belongs to.
        */
        tFileNameID = tf->index;
    ...
    /* No tags or tips files in the list, or the given file wasn't found */
    if (table == NULL || tFileNameID == 0) 
        return NULL;

    /* Allocate string */
    nameList = XtMalloc(sizeof(char) * allocLen);
    fillPtr = nameList;
    for (i = 0; i < DefTagHashSize; i++) {
        for (t = table[i]; t; t = t->next) {
            /* For any tags/tips file a unique identifier is created and added to
               the tag structure as t(ag)->index.  So, we know which tags/tips file
               contains the tag/tip (key/name).  */
            if (t->index != tFileNameID) 
                continue;
            
            if (t->name != NULL) {
                nameLen = strlen(t->name);
                if (stub != NULL) {
                    /* If we have a stub, we only add those tips/tag names that
                       are extensions of stub */
                    stubLen = strlen(stub);
                    if (stubLen < nameLen) {
                        nameCut = malloc(sizeof(char) * (stubLen + 1));
                        strncpy(nameCut, t->name, stubLen);
                        ptr = nameCut + stubLen;
                        *ptr = '\0';
                        if (strcmp(nameCut, stub)) {
                            free(nameCut);
                            continue;
                        }
                        free(nameCut);
                ...
                nameListLen += nameLen;
                nameListLen++;
                if (nameListLen > allocLen) {
                    allocLen += 1024;
                    nameList = XtRealloc(nameList, allocLen);
                    fillPtr = nameList;
                    fillPtr += nameListLen - nameLen - 1;
                }
                memcpy(fillPtr, t->name, nameLen);
                fillPtr += nameLen;
                *fillPtr = '\n';
                fillPtr++;
            }
    ...
    if (fillPtr == nameList) {
        /* in this case we haven't found anything */
        XtFree(nameList);
        return NULL;
    }
    /* remove last \n, and add a terminating \0 */
    fillPtr--;
    *fillPtr = '\0';

    /* nameList should be freed() in the calling function */
    return nameList;
}
...
**
**  (*) If nameList != NULL this is limited to N tags files which names are
**  given in nameList[0 .. N - 1]. The end of the list nameList[N] must be
**  marked by setting it to NULL.
**
**  Return value: total number of currently loaded tags files
*/    
...
static int loadAllTagsFiles(const char **nameList, int search_type) 
{
    ...
    */    
    for (tf = FileList; tf; tf = tf->next) {
        if (nameList) {
            const char **nameListEntry;
            int skipThisFile;
           
            skipThisFile = 1;
            for(nameListEntry=nameList; *nameListEntry; nameListEntry++) {
                if(!strcmp(*nameListEntry, tf->filename)) {
                           skipThisFile = 0;
                   break;                
                }
            }
            if(skipThisFile)
                continue; /* ... try next from FileList */
        }
              ...
              int search_type)
{
    ...
    */    
    if (name)
        loadAllTagsFiles(NULL, search_type);
...
**
** If search_type = TAG,
** returns <= 0 on errors or when no Def found, > 0 when Def found.
** Else,
** returns the calltip ID, or 0 on failure.
*/
static int findDef(WindowInfo *window, const char *value, int search_type) {
    ...
    if (l <= MAX_TAG_LEN) {
        ...
        if (!(*p)) {
            ...
            if (search_type == TAG && status == 0) {
                status++;
            }
            /* If we didn't find a requested calltip, see if we can use a tag */
            if (status == -1 && search_type == TIP && TagsFileList != NULL) {
...
/*      Finds all matches and handles tag "collisions". Prompts user with a 
        ...
        Returns -2 on error condition, -1 if no matches found and a
        number >= 0 if matches were found.
        (Note: the value is *not* the number of matches.) */
static int findAllMatches(WindowInfo *window, const char *string)
{
    ...
    */
    if (searchMode == TAG) {
        ...
        return showMatchingCalltip( dialogParent, 0 );
    }
...
** Returns the calltip ID or 0 in case of failure.
*/ 
static int showMatchingCalltip( Widget parent, int i )
{
    int startPos=0, fileLen, readLen, tipLen;
    int endPos=0, calltipID = 0;
    char *fileString;
    FILE *fp;
    struct stat statbuf;
    char *message;
    ...
    /* 6. Display it */
    calltipID = tagsShowCalltip( WidgetToWindow(parent), message );
    XtFree(message);
    XtFree(fileString);
    return calltipID;
}
        ...
        int *currLine) 
{
    ...
    if( dummy1 || searchLine(line, alias_regex) ) {
        ...
        /* ftell returns a long int, but incPos is declared as int,
           so the result from ftell is truncated! */
        incPos = ftell(fp);
        *blkLine = *currLine + 1; /* Line of first actual filename/alias */
        ...
        /* ftell returns long, but incPos and incLen are int */
...
/* 
 * Successive calls of this function return the names
 * of all loaded tips files in *tfName. 
 * For the first call *callData has to be NULL, the following
 * calls have to re-use the last value supplied by this function.
 * Return value: 1 there are further tips file names to be returned later
 *               0 the last name has been returned. Successive calls will
 *                 restart with the first tips file. 
 */
int GetNextTipsFileName(const char ** tfName, void ** callData) 
{
   tagFile * tipsFilePtr =  (tagFile *) * callData;
  
   if(!tipsFilePtr) 
      tipsFilePtr = TipsFileList;
   else
      tipsFilePtr = tipsFilePtr->next;
   
   *callData = tipsFilePtr;
   
   if(tipsFilePtr) {
      *tfName = tipsFilePtr->filename;
      return 1;
   } else {
      *tfName = "";
      return 0;
