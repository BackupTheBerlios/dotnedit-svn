...
static void moveTabToLeftOf(WindowInfo *move, WindowInfo *leftOf);
...
/*
** move a tab to one position to the right by tab order
*/
void MoveDocumentRight(WindowInfo *window)
{
    WindowInfo *rightOf;
    ...
    rightOf = getNextTabWindow(window, 1, GetPrefGlobalTabNavigate(), 1);
    if (rightOf == NULL)
    	return;

    moveTabToRightOf(window, rightOf);
}

/*
** move a tab to one position to the left by tab order
*/
void MoveDocumentLeft(WindowInfo *window)
{
    WindowInfo *leftOf;
    ...
    leftOf = getNextTabWindow(window, -1, GetPrefGlobalTabNavigate(), 1);
    if (leftOf == NULL)
    	return;

    moveTabToLeftOf(window, leftOf);
}
...
/*
** move a tab to one position to the left of another tab.
*/
void moveTabToLeftOf(WindowInfo *move, WindowInfo *leftOf)
{
    int tabCount, i;
    WidgetList tabList;
    WindowInfo *win;
    int movePos, leftOfPos;

    if (move == leftOf || move->shell != leftOf->shell)
        return;

    movePos = getTabPosition(move->tab);
    leftOfPos = getTabPosition(leftOf->tab);
    if (leftOfPos == movePos+1)
        return;

    XtVaGetValues(leftOf->tabBar, XmNtabWidgetList, &tabList,
            XmNtabCount, &tabCount, NULL);
    if (movePos < leftOfPos) {
        /* clockwise rotate, leftOf-tab exclusive*/
        for (i=movePos; i<leftOfPos-1; i++) {
            win = TabToWindow(tabList[i+1]);
            win->tab = tabList[i];
            RefreshTabState(win);
        }
        move->tab = tabList[leftOfPos-1];
        RefreshTabState(move);
    }
    else {
        /* anti-clockwise rotate, leftOf-tab inclusive  */
        for (i=movePos; i>leftOfPos; i--) {
            win = TabToWindow(tabList[i-1]);
            win->tab = tabList[i];
            RefreshTabState(win);
        }
        move->tab = tabList[leftOfPos];
        RefreshTabState(move);
    }
}
