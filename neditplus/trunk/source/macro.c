...
static int replaceSubstringMS(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg);
static int findCompletionsMS(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg);
...
static int killCalltipMS(WindowInfo *window, DataValue *argList, int nArgs,
       DataValue *result, char **errMsg);
/* T Balinski */
static int listDialogBase(WindowInfo *window, DataValue *argList, int nArgs,
      DataValue *result, char **errMsg, Boolean isMultiLine);
static int listDialogMS(WindowInfo *window, DataValue *argList, int nArgs,
	DataValue *result, char **errMsg);
static int listMultiselDialogMS(WindowInfo *window, DataValue *argList,
...
static int wrapMarginMV(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg);
static int topLeftMV(WindowInfo *window, DataValue *argList, int nArgs,
    DataValue *result, char **errMsg);
...
static int calltipIDMV(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg);
static int loadedTagsFilesMV(WindowInfo *window, DataValue *argList, 
	      int nArgs, DataValue *result, char **errMsg);
static int loadedTipsFilesMV(WindowInfo *window, DataValue *argList, 
	      int nArgs, DataValue *result, char **errMsg);
static int marksMV(WindowInfo *window, DataValue *argList, int nArgs,
        DataValue *result, char **errMsg);
static int commandShellMV(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg);
...
static BuiltInSubr MacroSubrs[] = {lengthMS, getRangeMS, tPrintMS,
        ...
        tolowerMS, listDialogMS, listMultiselDialogMS, getenvMS,
        stringCompareMS, splitMS, calltipMS, killCalltipMS,
        setBacklightStringMS, findCompletionsMS,
...
static const char *MacroSubrNames[N_MACRO_SUBRS] = {"length", "get_range", "t_print",
        ...
        "toupper", "tolower", "list_dialog", "list_multisel_dialog", "getenv",
        "string_compare", "split", "calltip", "kill_calltip",
        "set_backlight_string", "find_completions",
...
static BuiltInSubr SpecialVars[] = {cursorMV, lineMV, columnMV,
        ...
        minFontWidthMV, maxFontWidthMV, topLeftMV, topLineMV, numDisplayLinesMV,
        displayWidthMV, activePaneMV, nPanesMV, emptyArrayMV,
        serverNameMV, calltipIDMV,
        backlightStringMV,
        rangesetListMV, versionMV,
        loadedTagsFilesMV, loadedTipsFilesMV, marksMV, commandShellMV
    };
...
static const char *SpecialVarNames[N_SPECIAL_VARS] = {"$cursor", "$line", "$column",
        ...
        "$min_font_width", "$max_font_width", "$top_left", "$top_line", "$n_display_lines",
        "$display_width", "$active_pane", "$n_panes", "$empty_array",
        "$server_name", "$calltip_ID",
        "$backlight_string",
        "$rangeset_list", "$VERSION",
        "$loaded_tags_files", "$loaded_tips_files", "$marks", "$command_shell"
    };
...
static const char* RedundantActions[] = {"open_dialog", "save_as_dialog",
    ...
    "file_menu_command",  "edit_menu_command", "search_menu_command"};
/* We add the new file/edit/search_menu_command to emulate the old behaviour */
...
static int findCompletionsMS(WindowInfo *window, DataValue *argList, int nArgs,
    	DataValue *result, char **errMsg)
{
    char stringStorage[TYPE_INT_STR_SIZE(int)], *mode, *fileName;
    char *stub, *nameList;
    int search_type;
    ...
    /* Read the mode either "tips" or "tags" */
    if (!readStringArg(argList[0], &mode, stringStorage, errMsg))
        return False;

    /* Read the filename  */
    if (!readStringArg(argList[1], &fileName, stringStorage, errMsg))
        return False;
    
    if (nArgs == 3) {
        if (!readStringArg(argList[2], &stub, stringStorage, errMsg))
            return False;
    }
    else {
        stub = NULL;
    ...
    nameList = listKeys(search_type, fileName, stub);
	
    /* Return the text as an allocated string */
    result->tag = STRING_TAG;
    if (nameList) {
        AllocNStringCpy(&result->val.str, nameList);
        XtFree(nameList);
    }
    else {
        AllocNStringCpy(&result->val.str, "");    
    }
    return True;
      ...
      DataValue *result, char **errMsg, Boolean isMultiLine)
{
    ...
    XtSetArg(al[ac], XmNlistVisibleItemCount, (nlines > 25) ? 25 : nlines); ac++;
    XtSetArg(al[ac], XmNokLabelString, s2=XmStringCreateSimple(btnLabel)); ac++;
    ...
    XtVaSetValues(XmSelectionBoxGetChild(dialog, XmDIALOG_LIST),
                XmNselectionPolicy,
                            isMultiLine ? XmMULTIPLE_SELECT : XmSINGLE_SELECT,
...
static int listDialogMS(WindowInfo *window, DataValue *argList, int nArgs,
      DataValue *result, char **errMsg)
{
    return listDialogBase(window, argList, nArgs, result, errMsg, False);
}

static int listMultiselDialogMS(WindowInfo *window, DataValue *argList,
      int nArgs, DataValue *result, char **errMsg)
{
    return listDialogBase(window, argList, nArgs, result, errMsg, True);
}
      ...
      XtPointer callData)
{
    ...
    if (n_sel < 0) {
      ...
      n_sel = 0;
        ...
        /* count up the total size of the result */
        int i;
        size_t len;
        char *cp;
        length = 0;
        for (i = 0; i < n_sel; i++) {
            sel_index = seltable[i] - 1;
            length += strlen((char *)text_lines[sel_index]) + 1;
        }
        /* allocate result */
        cp = text = AllocString(length);
        /* and copy strings, separated by '\n' */
        for (i = 0; i < n_sel; i++) {
            sel_index = seltable[i] - 1;
            len = strlen((char *)text_lines[sel_index]);
            strcpy(cp, text_lines[sel_index]);
            cp += len;
            *cp++ = '\n';
        }
        text[--length] = '\0';
...
{
    result->tag = STRING_TAG;
    AllocNStringCpy(&result->val.str, GetPrefShell());
    return True;
}
...
{
    result->tag = INT_TAG;
    result->val.n = TextFirstVisiblePos(window->lastFocus);
    return True;
}
...
{   
    ...
    while (GetNextTagsFileName(&tfName, &callData)) {
        sprintf(indexStr, "%d", indexNum++);
        allocIndexStr = AllocString(strlen(indexStr) + 1);
        ...
        strcpy(allocIndexStr, indexStr);
        element.tag = STRING_TAG;
        if (!AllocNStringCpy(&element.val.str, tfName)) {
            M_FAILURE("Failed to allocate element value: %s");
            return(False);
        }
        if (!ArrayInsert(result, allocIndexStr, &element)) {
...
{   
    ...
    result->tag = ARRAY_TAG;
    result->val.arrayPtr = ArrayNew();
    while (GetNextTipsFileName(&tfName, &callData)) {
        sprintf(indexStr, "%d", indexNum++);
        allocIndexStr = AllocString(strlen(indexStr) + 1);
        if (allocIndexStr == NULL) {
            M_FAILURE("Failed to allocate array key in %s");
        ...
        DataValue *result, char **errMsg)
{
    ...
    for (i = 0; i < window->nMarks; i++) {
        element.tag = INT_TAG;
        element.val.n = window->markTable[i].cursorPos;
        sprintf(indexStr, "%c", tolower(window->markTable[i].label));
        allocIndexStr = AllocString(strlen(indexStr) + 1);
        if (allocIndexStr == NULL)
            M_FAILURE("Failed to allocate array key in %s");
        strcpy(allocIndexStr, indexStr);
