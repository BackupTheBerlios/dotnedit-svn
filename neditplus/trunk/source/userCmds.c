...
enum dialogTypes {FILE_CMDS, EDIT_CMDS, SEARCH_CMDS, SHELL_CMDS, MACRO_CMDS, BG_MENU_CMDS};
...
typedef struct {
    int dialogType;
    WindowInfo *window;
    Widget nameTextW, accTextW, accPrefixTextW, mneTextW, cmdTextW, saveFirstBtn;
    Widget loadAfterBtn, selInpBtn, makeSepBtn, hideItemBtn, winInpBtn, eitherInpBtn, noInpBtn;
...
/* Descriptions of the current user programmed menu items for re-generating
   menus and processing file, edit, search, shell, macro, and background 
   menu selections */
static menuItemRec *ShellMenuItems[MAX_ITEMS_PER_MENU], *MacroMenuItems[MAX_ITEMS_PER_MENU],
            *BGMenuItems[MAX_ITEMS_PER_MENU], *FileMenuItems[MAX_ITEMS_PER_MENU],
            *EditMenuItems[MAX_ITEMS_PER_MENU], *SearchMenuItems[MAX_ITEMS_PER_MENU];

static userMenuInfo  *ShellMenuInfo[MAX_ITEMS_PER_MENU], *MacroMenuInfo[MAX_ITEMS_PER_MENU],
            *BGMenuInfo[MAX_ITEMS_PER_MENU], *FileMenuInfo[MAX_ITEMS_PER_MENU],
            *EditMenuInfo[MAX_ITEMS_PER_MENU], *SearchMenuInfo[MAX_ITEMS_PER_MENU];

static userSubMenuCache  ShellSubMenus, MacroSubMenus, BGSubMenus, FileSubMenus,
            EditSubMenus, SearchSubMenus;

static int NShellMenuItems = 0, NMacroMenuItems = 0, NBGMenuItems = 0, 
            NFileMenuItems = 0, NEditMenuItems = 0, NSearchMenuItems = 0;


/* Top level shells of the user-defined menu editing dialogs */
static Widget ShellCmdDialog = NULL, MacroCmdDialog = NULL, BGMenuCmdDialog = NULL,
            FileCmdDialog = NULL, EditCmdDialog = NULL, SearchCmdDialog = NULL;
...
static Widget MacroPasteReplayBtn = NULL, BGMenuPasteReplayBtn = NULL, 
            FilePasteReplayBtn = NULL, EditPasteReplayBtn = NULL,
            SearchPasteReplayBtn = NULL;

static void editUserDefinedMenus(WindowInfo *window, int dialogType);
...
static Widget createUserFakeMenuItem(Widget menuPane, char *name,
	int index, XtCallbackProc cbRtn, XtPointer cbArg);
...
static void fileMenuCB(Widget w, WindowInfo *window, XtPointer callData);
static void editMenuCB(Widget w, WindowInfo *window, XtPointer callData);
static void searchMenuCB(Widget w, WindowInfo *window, XtPointer callData);
...
void EditShellMenu(WindowInfo *window)
{
    ...
    XmStringFree(s1);
    ucd->makeSepBtn = XtVaCreateManagedWidget("makeSepBtn",
    	    xmToggleButtonWidgetClass, form,
    	    XmNlabelString, s1=MKSTRING("Separator over Item"),
    	    XmNmnemonic, 'S',
    	    XmNalignment, XmALIGNMENT_BEGINNING,
    	    XmNmarginHeight, 1,
    	    XmNset, False,
	    XmNrightAttachment, XmATTACH_WIDGET,
	    XmNrightPosition, ucd->repInpBtn,
	    XmNrightOffset, 4,
	    XmNbottomAttachment, XmATTACH_WIDGET,
	    XmNbottomWidget, ucd->saveFirstBtn, NULL);
    XmStringFree(s1);
...
{
    editUserDefinedMenus(window, MACRO_CMDS);
}
void EditBGMenu(WindowInfo *window)
{
    editUserDefinedMenus(window, BG_MENU_CMDS);
}
void EditFileMenu(WindowInfo *window)
{
    editUserDefinedMenus(window, FILE_CMDS);
}
void EditEditMenu(WindowInfo *window)
{
    editUserDefinedMenus(window, EDIT_CMDS);
}
void EditSearchMenu(WindowInfo *window)
{
    editUserDefinedMenus(window, SEARCH_CMDS);
}
...
{   
    ...
    if (dialogType == FILE_CMDS && FileCmdDialog != NULL) {
    	RaiseDialogWindow(FileCmdDialog);
    	return;
    }
    if (dialogType == EDIT_CMDS && EditCmdDialog != NULL) {
    	RaiseDialogWindow(EditCmdDialog);
    	return;
    }
    if (dialogType == SEARCH_CMDS && SearchCmdDialog != NULL) {
    	RaiseDialogWindow(SearchCmdDialog);
    	return;
    }
    ...
    } else if (dialogType == BG_MENU_CMDS){ 
	for (i=0; i<NBGMenuItems; i++)
    	    ucd->menuItemsList[i] = copyMenuItemRec(BGMenuItems[i]);
	ucd->nMenuItems = NBGMenuItems;
        title = "Window Background Menu";
    } else if (dialogType == FILE_CMDS) {
        for (i=0; i<NFileMenuItems; i++)
            ucd->menuItemsList[i] = copyMenuItemRec(FileMenuItems[i]);
        ucd->nMenuItems = NFileMenuItems;
        title = "File Menu";
    } else if (dialogType == EDIT_CMDS) {
        for (i=0; i<NEditMenuItems; i++)
            ucd->menuItemsList[i] = copyMenuItemRec(EditMenuItems[i]);
        ucd->nMenuItems = NEditMenuItems;
        title = "Edit Menu";
    } else if (dialogType == SEARCH_CMDS) {
        for (i=0; i<NSearchMenuItems; i++)
            ucd->menuItemsList[i] = copyMenuItemRec(SearchMenuItems[i]);
        ucd->nMenuItems = NSearchMenuItems;
        title = "Search Menu";
    }
    ...
    ucd->makeSepBtn = XtVaCreateManagedWidget("makeSepBtn",
	    ...
	    XmNtopAttachment, XmATTACH_WIDGET,
            XmNtopWidget, ucd->selInpBtn,
	    XmNtopOffset, 1, NULL);
    XmStringFree(s1);
 
    ucd->hideItemBtn = XtVaCreateManagedWidget("hideItemBtn",
	    xmToggleButtonWidgetClass, form,
    	    XmNlabelString, s1=MKSTRING("Hidden"),
    	    XmNmnemonic, 'S',
    	    XmNalignment, XmALIGNMENT_BEGINNING,
    	    XmNmarginHeight, 0,
    	    XmNset, False,
	    XmNleftAttachment, XmATTACH_WIDGET,
	    XmNleftWidget, ucd-> selInpBtn,
	    XmNleftOffset, 1,
	    XmNbottomAttachment, XmATTACH_WIDGET,
            XmNbottomWidget, ucd->makeSepBtn,
	    XmNbottomOffset, 1, NULL);
    XmStringFree(s1);


    ucd->accPrefixTextW = XtVaCreateManagedWidget("accpref", xmTextWidgetClass, form,
	    XmNcolumns, 5,
	    XmNmaxLength, MAX_PREF_LEN-1,
	    XmNleftAttachment, XmATTACH_POSITION,
	    XmNleftPosition, RIGHT_MARGIN_POS-17,
	    XmNrightAttachment, XmATTACH_POSITION,
	    XmNrightPosition, RIGHT_MARGIN_POS,
	    XmNbottomAttachment, XmATTACH_WIDGET,
    	    XmNbottomWidget, ucd->selInpBtn,
	    XmNbottomOffset, 5, NULL);
    RemapDeleteKey(ucd->accPrefixTextW);
    ...
    XtVaCreateManagedWidget("accprefLabel", xmLabelGadgetClass, form,
    	    XmNlabelString, s1=MKSTRING("Prefix Text"),
    	    XmNmnemonic, 'f',
    	    XmNuserData, ucd->accPrefixTextW,
    	    XmNalignment, XmALIGNMENT_END,
    	    XmNmarginTop, 5,
    	    XmNleftAttachment, XmATTACH_POSITION,
    	    XmNleftPosition, LIST_RIGHT + 22,
    	    XmNrightAttachment, XmATTACH_POSITION,
    	    XmNrightPosition, RIGHT_MARGIN_POS-7,
    	    XmNbottomAttachment, XmATTACH_WIDGET,
    	    XmNbottomWidget, ucd->mneTextW, NULL);
    XmStringFree(s1);
...
Select \"New\" to add a new command to the menu."),
	...
	BGMenuCmdDialog = ucd->dlogShell;
        ...
        FileCmdDialog = ucd->dlogShell;
        FilePasteReplayBtn = pasteReplayBtn;
    }
    else if (dialogType == EDIT_CMDS) {
        EditCmdDialog = ucd->dlogShell;
        EditPasteReplayBtn = pasteReplayBtn;
    }
    else if (dialogType == SEARCH_CMDS) {
        SearchCmdDialog = ucd->dlogShell;
        SearchPasteReplayBtn = pasteReplayBtn;
    }
...
void UpdateUserMenus(WindowInfo *window)
{
    ...
    if (window->userMenuCache->umcLanguageMode != window->languageMode) {
        ...
        updateMenu(window, FILE_CMDS);
        updateMenu(window, EDIT_CMDS);
        updateMenu(window, SEARCH_CMDS);
#ifndef VMS
...
void DimPasteReplayBtns(int sensitive)
{
    ...
    if (FileCmdDialog != NULL)
        XtSetSensitive(FilePasteReplayBtn, sensitive);
    if (EditCmdDialog != NULL)
        XtSetSensitive(EditPasteReplayBtn, sensitive);
    if (SearchCmdDialog != NULL)
        XtSetSensitive(SearchPasteReplayBtn, sensitive);
}
...
void DimSelectionDepUserMenuItems(WindowInfo *window, int sensitive)
{
    ...
    dimSelDepItemsInMenu(window->fileMenuPane, FileMenuItems,
	    NFileMenuItems, sensitive);
    dimSelDepItemsInMenu(window->editMenuPane, EditMenuItems,
	    NEditMenuItems, sensitive);
    dimSelDepItemsInMenu(window->searchMenuPane, SearchMenuItems,
	    NSearchMenuItems, sensitive);
}
	...
	int nMenuItems, int sensitive)
{
	...
	XtVaGetValues(items[n], XmNuserData, &userData, NULL);
		...
		dimSelDepItemsInMenu(subMenu, menuList, nMenuItems, sensitive);
                ...
                if (XtClass(items[n]) != xmSeparatorWidgetClass) {
...
char *WriteFileCmdsString(void)
{
    return writeMenuItemString(FileMenuItems, NFileMenuItems, FILE_CMDS);
}
char *WriteEditCmdsString(void)
{
    return writeMenuItemString(EditMenuItems, NEditMenuItems, EDIT_CMDS);
}
char *WriteSearchCmdsString(void)
{
    return writeMenuItemString(SearchMenuItems, NSearchMenuItems, SEARCH_CMDS);
}
...
int LoadFileCmdsString(char *inString)
{
    return loadMenuItemString(inString, FileMenuItems, &NFileMenuItems,
    	    FILE_CMDS);
}

int LoadEditCmdsString(char *inString)
{
    return loadMenuItemString(inString, EditMenuItems, &NEditMenuItems,
    	    EDIT_CMDS);
}

int LoadSearchCmdsString(char *inString)
{
    return loadMenuItemString(inString, SearchMenuItems, &NSearchMenuItems,
    	    SEARCH_CMDS);
}
...
void SetupUserMenuInfo(void)
{
    ...
    parseMenuItemList(FileMenuItems, NFileMenuItems, FileMenuInfo, &FileSubMenus);
    parseMenuItemList(EditMenuItems, NEditMenuItems, EditMenuInfo, &EditSubMenus);
    parseMenuItemList(SearchMenuItems, NSearchMenuItems, SearchMenuInfo, &SearchSubMenus);  
}
...
void UpdateUserMenuInfo(void)
{
    ...
    freeUserMenuInfoList(FileMenuInfo, NFileMenuItems);
    freeSubMenuCache(&FileSubMenus);
    parseMenuItemList(FileMenuItems, NFileMenuItems, FileMenuInfo, &FileSubMenus);

    freeUserMenuInfoList(EditMenuInfo, NEditMenuItems);
    freeSubMenuCache(&EditSubMenus);
    parseMenuItemList(EditMenuItems, NEditMenuItems, EditMenuInfo, &EditSubMenus);

    freeUserMenuInfoList(SearchMenuInfo, NSearchMenuItems);
    freeSubMenuCache(&SearchSubMenus);
    parseMenuItemList(SearchMenuItems, NSearchMenuItems, SearchMenuInfo, &SearchSubMenus);
}
...
int DoNamedFileMenuCmd(WindowInfo *window, const char *itemName)
{
    int i;
    
    for (i=0; i<NFileMenuItems; i++) {
    	if (!strcmp(FileMenuItems[i]->name, itemName)) {
    	    DoMacro(window, FileMenuItems[i]->cmd, "file menu command");
    	    return True;
    	}
    }
    return False;
...
int DoNamedEditMenuCmd(WindowInfo *window, const char *itemName)
{
    int i;
    
    for (i=0; i<NEditMenuItems; i++) {
    	if (!strcmp(EditMenuItems[i]->name, itemName)) {
    	    DoMacro(window, EditMenuItems[i]->cmd, "edit menu command");
    	    return True;
    	}
    }
    return False;
...
int DoNamedSearchMenuCmd(WindowInfo *window, const char *itemName)
{
    int i;
    
    for (i=0; i<NSearchMenuItems; i++) {
    	if (!strcmp(SearchMenuItems[i]->name, itemName)) {
    	    DoMacro(window, SearchMenuItems[i]->cmd, "search menu command");
    	    return True;
    	}
    }
    return False;
...
/*
** Cache user menus:
** Rebuild all of the Shell, Macro, Background, File, Search, Edit menus of given editor window.
*/
void RebuildAllMenus(WindowInfo *window)
{
     ...
     rebuildMenu(window, FILE_CMDS);
     rebuildMenu(window, EDIT_CMDS);
     rebuildMenu(window, SEARCH_CMDS);
}
...
static void selectUserMenu(WindowInfo *window, int menuType, selectedUserMenu *menu)
{
    ...
    } else if (menuType == FILE_CMDS) {
        menu->sumMenuPane       = window->fileMenuPane;
        menu->sumNbrOfListItems = NFileMenuItems;
        menu->sumItemList       = FileMenuItems;
        menu->sumInfoList       = FileMenuInfo;
        menu->sumSubMenus       = &FileSubMenus;
        menu->sumMainMenuList   = &window->userMenuCache->umcFileMenuList;
        menu->sumMenuCreated    = &window->userMenuCache->umcFileMenuCreated;
    } else if (menuType == EDIT_CMDS) {
        menu->sumMenuPane       = window->editMenuPane;
        menu->sumNbrOfListItems = NEditMenuItems;
        menu->sumItemList       = EditMenuItems;
        menu->sumInfoList       = EditMenuInfo;
        menu->sumSubMenus       = &EditSubMenus;
        menu->sumMainMenuList   = &window->userMenuCache->umcEditMenuList;
        menu->sumMenuCreated    = &window->userMenuCache->umcEditMenuCreated;
    } else if (menuType == SEARCH_CMDS) {
        menu->sumMenuPane       = window->searchMenuPane;
        menu->sumNbrOfListItems = NSearchMenuItems;
        menu->sumItemList       = SearchMenuItems;
        menu->sumInfoList       = SearchMenuInfo;
        menu->sumSubMenus       = &SearchSubMenus;
        menu->sumMainMenuList   = &window->userMenuCache->umcSearchMenuList;
        menu->sumMenuCreated    = &window->userMenuCache->umcSearchMenuCreated;
    }
...
static void createMenuItems(WindowInfo *window, selectedUserMenu *menu)
{
    ...
    char *accKeys;
    void (*cbTmp)(Widget w, WindowInfo *window, XtPointer callData);
    ...
    */
	...
	for (;;) {
	    ...
	    if (subSep == NULL) {
                ...
                /* place separator over menu entry */
                if (item->separator == MAKE_SEPARATOR) {
                    createMenuSeparator(subPane, " ", SHORT);
                }

		if (item->hidden == MAKE_HIDDEN) {
                    btn = createUserFakeMenuItem(subPane, namePtr, n,
			    (XtCallbackProc)cbTmp, (XtPointer)window);
                }
                else {
...
{
    XmString st1, st2;
    char accText[MAX_ACCEL_LEN], *accPrefixText=malloc((MAX_ACCEL_LEN + MAX_PREF_LEN) * sizeof(char));
    Widget btn;
    
    generateAcceleratorString(accText, f->modifiers, f->keysym);

    /* Add possible prefix, eg, [Shift], to the Button description only */
/* && accText[0] != '\0' */
    if (cbRtn != (XtCallbackProc)shellMenuCB) {
        strcpy(accPrefixText, f->prefix);
        strcat(accPrefixText, accText);
        strncpy(accText, accPrefixText, MAX_ACCEL_LEN);
        free(accPrefixText);
    }
	...
	int index, XtCallbackProc cbRtn, XtPointer cbArg)
{
    ...
    XtAddCallback(btn, XmNactivateCallback, cbRtn, cbArg);
    XmStringFree(st1);
    XtVaSetValues(btn, XmNtraversalOn, False, NULL);
    return btn;
}
...
static void closeCB(Widget w, XtPointer clientData, XtPointer callData)
{
    ...
    if (ucd->dialogType == SHELL_CMDS)
    	...
    	MacroCmdDialog = NULL;
    else if (ucd->dialogType == BG_MENU_CMDS)
	BGMenuCmdDialog = NULL;
    else if (ucd->dialogType == FILE_CMDS)
        FileCmdDialog = NULL;
    else if (ucd->dialogType == EDIT_CMDS)
        EditCmdDialog = NULL;
    else if (ucd->dialogType == SEARCH_CMDS)
        SearchCmdDialog = NULL;
...
static int applyDialogChanges(userCmdDialog *ucd)
{
    ...
    if (ucd->dialogType == SHELL_CMDS) {
    	...
    	for (i=0; i<NFileMenuItems; i++)
    	    freeMenuItemRec(FileMenuItems[i]);
        freeUserMenuInfoList(FileMenuInfo, NFileMenuItems);
        freeSubMenuCache(&FileSubMenus);
    	for (i=0; i<ucd->nMenuItems; i++)
    	    FileMenuItems[i] = copyMenuItemRec(ucd->menuItemsList[i]);
    	NFileMenuItems = ucd->nMenuItems;
        parseMenuItemList(FileMenuItems, NFileMenuItems, FileMenuInfo, &FileSubMenus);
    } else if (ucd->dialogType == EDIT_CMDS) {
    	for (i=0; i<NEditMenuItems; i++)
    	    freeMenuItemRec(EditMenuItems[i]);
        freeUserMenuInfoList(EditMenuInfo, NEditMenuItems);
        freeSubMenuCache(&EditSubMenus);
    	for (i=0; i<ucd->nMenuItems; i++)
    	    EditMenuItems[i] = copyMenuItemRec(ucd->menuItemsList[i]);
    	NEditMenuItems = ucd->nMenuItems;
        parseMenuItemList(EditMenuItems, NEditMenuItems, EditMenuInfo, &EditSubMenus);
    } else if (ucd->dialogType == SEARCH_CMDS) {
    	for (i=0; i<NSearchMenuItems; i++)
    	    freeMenuItemRec(SearchMenuItems[i]);
        freeUserMenuInfoList(SearchMenuInfo, NSearchMenuItems);
        freeSubMenuCache(&SearchSubMenus);
    	for (i=0; i<ucd->nMenuItems; i++)
    	    SearchMenuItems[i] = copyMenuItemRec(ucd->menuItemsList[i]);
    	NSearchMenuItems = ucd->nMenuItems;
        parseMenuItemList(SearchMenuItems, NSearchMenuItems, SearchMenuInfo, &SearchSubMenus);
    }
...
static void fileMenuCB(Widget w, WindowInfo *window, XtPointer callData) 
{
    ...
    XtVaGetValues(w, XmNuserData, &userData, NULL);
    index = (int)userData - 10;
    if (index <0 || index >= NFileMenuItems)
    	return;
    
    params[0] = FileMenuItems[index]->name;
    XtCallActionProc(window->lastFocus, "file_menu_command",
    	    ((XmAnyCallbackStruct *)callData)->event, params, 1);
}
...
static void editMenuCB(Widget w, WindowInfo *window, XtPointer callData) 
{
    ...
    XtVaGetValues(w, XmNuserData, &userData, NULL);
    index = (int)userData - 10;
    if (index <0 || index >= NEditMenuItems)
    	return;
    
    params[0] = EditMenuItems[index]->name;
    XtCallActionProc(window->lastFocus, "edit_menu_command",
    	    ((XmAnyCallbackStruct *)callData)->event, params, 1);
}
...
static void searchMenuCB(Widget w, WindowInfo *window, XtPointer callData) 
{
    ...
    XtVaGetValues(w, XmNuserData, &userData, NULL);
    index = (int)userData - 10;
    if (index <0 || index >= NSearchMenuItems)
    	return;
    
    params[0] = SearchMenuItems[index]->name;
    XtCallActionProc(window->lastFocus, "search_menu_command",
    	    ((XmAnyCallbackStruct *)callData)->event, params, 1);
}
...
static void updateDialogFields(menuItemRec *f, userCmdDialog *ucd)
{
    char mneString[2], prefString[MAX_PREF_LEN], accString[MAX_ACCEL_LEN];
    int i;
	...
	if (ucd->dialogType != SHELL_CMDS) {
            XmTextSetString(ucd->accPrefixTextW, "");
        }
	if (ucd->dialogType == SHELL_CMDS) {
	...
	if (ucd->dialogType != SHELL_CMDS) {
            for (i=0; i<MAX_PREF_LEN; i++)
                 prefString[i] = f->prefix[i];
            XmTextSetString(ucd->accPrefixTextW, prefString);
        }
	...
	if (ucd->dialogType == SHELL_CMDS) {
        ...
        RadioButtonChangeState(ucd->makeSepBtn, f->separator==MAKE_SEPARATOR, False);

        /* new hide button not for shell menu */
        if (ucd->dialogType != SHELL_CMDS) {
            RadioButtonChangeState(ucd->hideItemBtn, f->hidden==MAKE_HIDDEN, False);
        }
...
static menuItemRec *readDialogFields(userCmdDialog *ucd, int silent)
{
    char *nameText, *cmdText, *mneText, *accText, *accPrefixText;
    menuItemRec *f;
    ...
    /* prefix musn't contain { to be able to read preferences file format of unpatched NEdit */
    if (ucd->dialogType != SHELL_CMDS) {
        accPrefixText = XmTextGetString(ucd->accPrefixTextW);
        if (strchr(accPrefixText, ':') || strchr(accPrefixText, '{')){
            if (!silent)
            {
                DialogF(DF_WARN, ucd->dlogShell, 1, "Prefix Entry",
                        "Prefix must not contain : or { characters", "OK");
                XmProcessTraversal(ucd->accPrefixTextW, XmTRAVERSE_CURRENT);
            }
            XtFree(accPrefixText);
            return NULL;
        }
        else {
            strncpy(f->prefix, accPrefixText, MAX_PREF_LEN);
            XtFree(accPrefixText);
        }
    }
    else {
	...
	f->loadAfter = XmToggleButtonGetState(ucd->loadAfterBtn);
        f->separator = XmToggleButtonGetState(ucd->makeSepBtn) ? MAKE_SEPARATOR :
		MAKE_NOTHING;
    } else {
		...
		MAKE_NOTHING;
        /* new hide button not for shell menu */
        f->hidden = XmToggleButtonGetState(ucd->hideItemBtn) ? MAKE_HIDDEN :
		MAKE_NOTHING;
    ...
    return TextWidgetIsBlank(ucd->nameTextW) &&
	    ...
	    (ucd->dialogType != SHELL_CMDS || (
	    	...
	    	XmToggleButtonGetState(ucd->sameOutBtn) &&
  	    	!XmToggleButtonGetState(ucd->makeSepBtn) &&   
	    	!XmToggleButtonGetState(ucd->repInpBtn) &&
	    	!XmToggleButtonGetState(ucd->saveFirstBtn) &&
	    	!XmToggleButtonGetState(ucd->loadAfterBtn)));
}
/* hideItemBtn not for shell menu */
else {
    return TextWidgetIsBlank(ucd->nameTextW) &&
 	    TextWidgetIsBlank(ucd->cmdTextW) &&
	    TextWidgetIsBlank(ucd->accTextW) &&
	    TextWidgetIsBlank(ucd->mneTextW) &&
	    TextWidgetIsBlank(ucd->accPrefixTextW) &&
	    (ucd->dialogType != SHELL_CMDS || (
	    	XmToggleButtonGetState(ucd->selInpBtn) &&
  	    	XmToggleButtonGetState(ucd->makeSepBtn) &&   
  	    	XmToggleButtonGetState(ucd->hideItemBtn) &&   
	    	XmToggleButtonGetState(ucd->sameOutBtn) &&
	    	!XmToggleButtonGetState(ucd->repInpBtn) &&
	    	!XmToggleButtonGetState(ucd->saveFirstBtn) &&
	    	!XmToggleButtonGetState(ucd->loadAfterBtn)));
	...
	int listType)
{
    ...
    for (i=0; i<nItems; i++) {
        ...
        if (listType != SHELL_CMDS)
            length += MAX_PREF_LEN;    /* strlen(f->prefix); */
    	...
    	*outPtr++ = ':';
    	if (listType == SHELL_CMDS) {
   		...
   		*outPtr++ = 'R';
            if (f->separator == MAKE_SEPARATOR)
                *outPtr++ = 'S';
            if (f->hidden == MAKE_HIDDEN)
                *outPtr++ = 'H';
	    *outPtr++ = ':';

            /* new prefix text not for shell menu */
            if (listType != SHELL_CMDS) {
                if (f->prefix[0] !='\0') {
                    strcpy(outPtr,f->prefix);
                    outPtr += strlen(f->prefix);
                }
                *outPtr++ = ':';
            }
	...
	int *nItems, int listType)
{
    menuItemRec *f;
    char *cmdStr;
    char *inPtr = inString, *tmp;
    char *nameStr, accStr[MAX_ACCEL_LEN], *accPrefStr, mneChar;
    KeySym keysym;
    unsigned int modifiers;
    int i, input, output, saveFirst, loadAfter, repInput, separator, hidden;
    int nameLen, accLen, accPrefLen, mneLen, cmdLen;
	...
	if (nameLen == 0)
    	...
    	inPtr += nameLen;
	if (*inPtr == '\0')
	    return parseError("name end not expected");
	inPtr++;
	...
	if (accLen >= MAX_ACCEL_LEN)
    	...
    	inPtr += accLen;
	if (*inPtr == '\0')
	    return parseError("accelerator end not expected");
    	inPtr++;
    	...
    	inPtr++;
    	if (*inPtr == '\0')
	    return parseError("mnemonic end not expected");
	...
	inPtr++;
        ...
        /* read prefix field for MACRO_CMDS */
        if (listType != SHELL_CMDS) {
            accPrefLen = strcspn(inPtr, ":{");
            tmp = inPtr;
            tmp += accPrefLen;
            /* load also preferences files of unpatched NEdit versions */
            if (*tmp == ':') {
                if (accPrefLen >= MAX_PREF_LEN)
                    return parseError("prefix field too long");
                accPrefStr = XtMalloc(accPrefLen+1);
                strncpy(accPrefStr, inPtr, accPrefLen);
                accPrefStr[accPrefLen] = '\0';
    	        inPtr += accPrefLen;
	        if (*inPtr == '\0')
	            return parseError("prefix end not expected");
                inPtr++;
            }
            else
                accPrefStr = "";
	...
	f->mnemonic = mneChar;
        if (listType != SHELL_CMDS) {
            strncpy(f->prefix, accPrefStr, MAX_PREF_LEN);
        }
        else {
            f->prefix[0] = '\0';
        }
...
UserMenuCache *CreateUserMenuCache(void)
{
    ...
    cache->umcFileMenuCreated           =  False;
    cache->umcEditMenuCreated           =  False;
    cache->umcSearchMenuCreated         =  False;
    ...
    cache->umcFileMenuList.umlNbrItems  =  0;
    cache->umcFileMenuList.umlItems     =  NULL;
    cache->umcEditMenuList.umlNbrItems  =  0;
    cache->umcEditMenuList.umlItems     =  NULL;
    cache->umcSearchMenuList.umlNbrItems=  0;
    cache->umcSearchMenuList.umlItems   =  NULL;
...
void FreeUserMenuCache(UserMenuCache *cache)
{
    ...
    freeUserMenuList(&cache->umcFileMenuList);
    freeUserMenuList(&cache->umcEditMenuList);
    freeUserMenuList(&cache->umcSearchMenuList);
