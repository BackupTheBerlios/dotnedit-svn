...
void UnsetMark(WindowInfo *window, Widget widget, char label)
{
    int index, i;
    
    if (window->nMarks > 0) {
        /* look for the mark to unset */
        label = toupper(label);
        for (index=0; index<window->nMarks; index++) {
    	    if (window->markTable[index].label == label)
   	        break;
        ...
        if (index < window->nMarks) {
            for (i = index; i < window->nMarks - 1; i++) {
                window->markTable[i] = window->markTable[i+1];
            }
            /* simply forget about the last one */
            window->nMarks--;
        }
    }
